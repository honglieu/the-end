...
import { ErrorMessages, trudiUserId } from '@/app/services/constants';
...
export interface IMessengerTriggerSendMsgEvent extends ISendMsgTriggerEvent {
  sendOption?: SendOption;
  tempIds?: string[];
  tempConversationId?: string;
}
@Component({
  selector: 'messenger-inline-editor',
  templateUrl: './messenger-inline-editor.component.html',
  styleUrls: ['./messenger-inline-editor.component.scss'],
  providers: [
...
export class MessengerInlineEditorComponent
  implements OnInit, OnChanges, OnDestroy
{
  ...
  @Input() configs: ISendMsgConfigs = {
    ...
    otherConfigs: {
      ...
      createMessageFrom: ECreateMessageFrom.MESSENGER
    },
    inputs: {
  ...
  @Output() onSendMsg = new EventEmitter<IMessengerTriggerSendMsgEvent>();
  ...
  public triggerResizeMessengerCompose$ = new Subject<boolean>();
  ...
  private isDisconnectMessenger: boolean = false;
  selectedFacebookMessenger: PageFacebookMessengerType;
  constructor(
  ...
  get isDisableSendBtn() {
    ...
    const isMessengerEnabled =
      this.features?.[EAiAssistantAction.MESSENGER]?.state;
    const isMailboxArchived =
      this.currentMailBox?.status === EMailBoxStatus.ARCHIVE;
    const isConsole = this.isConsole;
    const isDisconnectMessenger = this.isDisconnectMessenger;
  ...
  ngOnInit(): void {
    ...
    this.facebookAccountService.currentPageMessengerActive$
      .pipe(takeUntil(this.destroy$))
      .subscribe((res) => {
        this.isDisconnectMessenger =
          res?.status === EPageMessengerConnectStatus.DISCONNECTED;
      });
  ...
  private handleAutoResizeSendMsgInline() {
    combineLatest([
      ...
      this.triggerResizeMessengerCompose$.pipe(startWith(false))
    ])
  ...
  private sendMessageV2({ body, tempConversationId, sendType }) {
    this.trudiSendMsgService.sendMessageV2(body).subscribe({
      ...
      complete: () => {
        ...
        this.triggerResizeMessengerCompose$.next(true);
      },
      error: (err) => {
        this.isLoading = false;
      ...
      .subscribe((res) => {
        ...
        if (!isAutoSaveDraft) {
          ...
          const linkPram = {
            ...
            path: '/inbox/facebook-messages/all',
            params: <{}>{
              status:
  ...
  private showToastMsg(type, numberOfMessages?, data?) {
    ...
    switch (type) {
      ...
      case ESentMsgEvent.ERR: {
        this.toastService.error(`${messageLabel}  failed to send`);
        break;
      }
