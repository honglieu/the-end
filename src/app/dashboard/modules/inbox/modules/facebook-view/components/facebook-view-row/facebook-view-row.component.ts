...
import { FacebookMenuService } from '@/app/dashboard/modules/inbox/modules/facebook-view/services/facebook-menu.service';
import { PluralizePipe } from '@/app/shared/pipes/pluralize';

@Component({
  selector: 'facebook-view-row',
  templateUrl: './facebook-view-row.component.html',
  styleUrl: './facebook-view-row.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class FacebookViewRowComponent
  implements OnInit, OnChanges, OnDestroy, AfterViewInit
{
  ...
  constructor(
    ...
    private readonly facebookMenuService: FacebookMenuService,
  ...
  ngOnInit(): void {
    ...
    this.facebookService.currentFacebookTask$
      .pipe(
        ...
        filter(
          (currentFacebookTask) =>
            !!currentFacebookTask && currentFacebookTask.id === this.item.id
        )
      )
      .subscribe((currentFacebookTask) => {
        this.item = {
          ...currentFacebookTask,
          status: this.item.status,
          conversationId: this.item.conversationId,
          conversations: currentFacebookTask.conversations.map(
            (conversation) => ({
              ...conversation,
  ...
  checkMessageStatus() {
    ...
    this.isUserVerified = this.item.conversations?.[0].isDetectedContact;
  }
  ...
  handleMenu(option: MenuOption, field: string, event?: MouseEvent): void {
    ...
    this.facebookMenuService
      ...
      .then((value) => {
        ...
        this.facebookService.setMenuRightClick({
          taskId: this.item.id,
  ...
  openToast(item: FacebookMessage, option: MenuOption) {
    const dataForToast = {
  ...
  handleChangeConversation(field: string, value: boolean) {
    if (this.queryTaskId === this.item?.id) {
      this.facebookService.setCurrentFacebookTask({
        ...this.item,
  ...
  onOpenTaskDetail(): void {
    const queryParams = {
      [FacebookQueryType.TASK_ID]: this.item.id,
    ...
    if (
      this.queryParams['conversationId'] === this.item.conversationId &&
      this.queryParams['taskId'] === this.item.id
    ) {
      return;
    }
