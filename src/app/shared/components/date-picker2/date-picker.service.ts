...
import { CompatibleDate, RangePartType, TrudiDateMode } from './standard-types';
...
export class DatePickerService implements OnDestroy {
  ...
  inputPartChange$ = new Subject<RangePartType | void>();

  dueDate: CandyDate | null = null;
  holidays = {};
  ...
  setActiveDate(
    ...
    const parentPanels: { [key in TrudiDateMode]?: NormalizedMode } = {
      date: 'month',
      month: 'year',
      year: 'decade'
    };
  ...
  setValue(value: CompatibleValue | any): void {
    this.value = value;
