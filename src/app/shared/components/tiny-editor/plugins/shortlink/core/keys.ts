...
import * as Options from '@shared/components/tiny-editor/plugins/shortlink/api/options';
import {
  findChar,
  freefallRtl,
  hasProtocol,
...
const parseCurrentLine = (
  ...
  if (!rngText.startsWith('http') && !rngText.startsWith('www')) {
    return null;
  }
  ...
  if (!beforeExecEvent.isDefaultPrevented()) {
    ...
    if (Type.isString(defaultLinkTarget)) {
      ...
      if (
        ...
        !Options.allowUnsafeLinkTarget(editor)
      ) {
