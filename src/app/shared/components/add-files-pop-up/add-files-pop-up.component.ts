...
export class AddFilesPopUpComponent implements OnInit {
  ...
  @Output() isBackSendMess = new EventEmitter<boolean>();
  @Output() isDoneAddFile = new EventEmitter<boolean>();
  ...
  public backtoSendMessageModal(status) {
    this.isValidFileUploadType = true;
    this.overFileSize = false;
    this.selectedFile = null;
    this.timeUpload = 0;
    this.isBackSendMess.emit(false);
    this.checkStatusQuoteFile(this.statusQuoteFile);
    this.popupService.setIsScroll(false);
  }

  public openSendMessageModal(status) {
    this.overFileSize = false;
    ...
    this.isDoneAddFile.emit(false);
  }

  public checkStatusQuoteFile(status) {
    if (status) {
      ...
      this.isOpenSendQuote.next(false);
    }
  }
