...
@Component({
  selector: 'select-inspection',
  templateUrl: './select-inspection.component.html',
  styleUrls: ['./select-inspection.component.scss']
})
export class SelectInspectionComponent implements OnInit, OnDestroy {
  ...
  getListInspection(taskId: string) {
    ...
    return this.ingoingInspectionSyncService
        ...
        tap((res) => {
          if (res.inspections.length) {
            ...
            this.listOfInspection = res.inspections.map((item) => {
              return {
                id: item?.id,
                label: `Ingoing Inspection - ${this.agencyDateFormatService.formatTimezoneDate(
                  item?.startTime,
                  DATE_FORMAT_DAYJS
                )} (${item?.status})`,
                group: '',
