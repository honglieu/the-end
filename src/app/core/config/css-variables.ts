/**
 * Sync from @trudi-design/colors(https://github.com/trudi-design/trudi-design-colors)
 */
import { TinyColor } from '@ctrl/tinycolor';

import { generate } from '@core';
import { warn } from '@core';
import { canUseDom, updateCSS } from '@core';
import { Theme } from './config';
import uuid4 from 'uuid4';

const dynamicStyleMark = `-trudi-${uuid4()}`;
...
export function registerTheme(globalPrefixCls: string, theme: Theme): void {
  ...
  if (canUseDom()) {
    ...
    warn(
      `TrudiConfigService: SSR do not support dynamic theme with css variables.`
    );
  }
}
