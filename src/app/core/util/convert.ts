...
function propDecoratorFactory<T, D>(
  name: string,
  fallback: (v: T) => D
): (target: TrudiSafeAny, propName: string) => void {
  function propDecorator(
    target: TrudiSafeAny,
    propName: string,
    originalDescriptor?: TypedPropertyDescriptor<TrudiSafeAny>
  ): TrudiSafeAny {
 ...
 * ```
 */
export function InputBoolean(): TrudiSafeAny {
  return propDecoratorFactory('InputBoolean', toBoolean);
}

export function InputCssPixel(): TrudiSafeAny {
  return propDecoratorFactory('InputCssPixel', toCssPixel);
}

export function InputNumber(fallbackValue?: TrudiSafeAny): TrudiSafeAny {
