 ...
 */
@Directive({
  selector: '[trudiMention], [mentionConfig]',
  // TODO: fix the issue with the host metadata property
...
export class TrudiMentionDirective implements OnChanges {
  ...
  @Input() set trudiMention(items: any[]) {
  ...
  private DEFAULT_CONFIG: MentionConfig = {
    ...
    mentionSelect: (item: any) => {
      return item[this.activeConfig.insertValue];
    },
  ...
  private startNode;
  private searchList: TrudiMentionComponent;
  private searching: boolean;
  private iframe: any; // optional
  private editorInstance: any; // optional
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['trudiMention'] || changes['mentionConfig']) {
      this.updateConfig();
    }
  }
  ...
  showSearchList(nativeElement: HTMLInputElement) {
    ...
    if (this.searchList == null) {
      let componentRef = this._viewContainerRef.createComponent(
      ...
      componentRef.instance['clickOutside'].subscribe((event) => {
        this.clickOutside.emit(event);
      });
    }
    this.searchList.labelKey = this.activeConfig.labelKey;
    this.searchList.avatarKey = this.activeConfig.avatarKey;
    this.searchList.styleOff = this.mentionConfig.disableStyle;
    this.searchList.activeIndex = this.activeConfig.items.findIndex(
      (item) => !item.disabled
    );
