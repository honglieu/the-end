...
import { IndexableObject, TrudiSafeAny, warn } from './logger';
...
export class CandyDate implements IndexableObject {
  ...
  private toNativeDate(date: TrudiSafeAny): Date {
    return date instanceof CandyDate ? date.nativeDate : date;
  }

  //Weeks start on Monday
  getWeekCountByMondayStart(): number {
    const year = this.getYear();
    const month_number = this.getMonth();
    const firstDay = new Date(year, month_number, 1).getDay();
    const totalDays = new Date(year, month_number + 1, 0).getDate() - 1;
    const total = (firstDay === 0 ? 7 : firstDay) + totalDays;
