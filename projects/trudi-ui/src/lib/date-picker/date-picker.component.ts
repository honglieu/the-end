...
import { PREFIX_CLASS, PREFIX_CLASS_SCHEDULE } from './util';
import { format, parse } from 'date-fns';
import { NzDatePickerSizeType, NzPlacement } from './custom';
...
@Component({
  ...
  template: `
    <ng-container *ngIf="!nzInline; else inlineMode">
      ...
      <div
        *ngIf="!isRange"
        class="{{ nzIsSchedule ? schedulePrefixCls : prefixCls }}-input">
        @if(nzIsSchedule){
        <div
          class="d-flex align-items-center gap-4 cursor-pointer schedule-date-picker-wrapper"
          (focus)="onFocus($event)"
          ...
          @if(errorMsg){
          <img
            src="/assets/icon/icon-schedule-v2-error.svg"
            alt="Error"
            (mousedown)="onImageClick($event)" />
          } @else {
          <img
            src="/assets/icon/icon-schedule-v2.svg"
            (mousedown)="onImageClick($event)" />
          }
          <input
            #pickerInput
            ...
            class="{{
              nzIsSchedule ? schedulePrefixCls : prefixCls
            }}-single-picker-input"
            [attr.id]="nzId"
      ...
      <span class="{{ prefixCls }}-suffix">
        ...
        <nz-form-item-feedback-icon
          *ngIf="hasFeedback && !!status"
          [status]="status"></nz-form-item-feedback-icon>
      </span>
    </ng-template>
  ...
  `,
  host: {
    '[class.ant-picker]': `!nzIsSchedule`,
    '[class.schedule-date-picker]': `nzIsSchedule`,
  ...
  providers: [
    DatePickerService,
    {
      ...
      multi: true,
      useExisting: forwardRef(() => TrudiUIDatePickerComponent)
    }
  ],
  styleUrls: ['./style/date-picker.scss'],
  animations: [slideMotion]
})
export class TrudiUIDatePickerComponent
  implements OnInit, OnChanges, OnDestroy, AfterViewInit, ControlValueAccessor
{
  ...
  @Input() @InputBoolean() nzIsSchedule: boolean = false;
  ...
  schedulePrefixCls = PREFIX_CLASS_SCHEDULE;
  ...
  onlyLeft: boolean = false; //Use for range only
  ...
  public placeHolder$ = this._placeholderSource$.asObservable().pipe(
    switchMap((dateFormat: string) => {
      return this.trudiDateFormat.dateFormatPipe$.pipe(
        map((defaultDateFormat) =>
          ((dateFormat || defaultDateFormat) as string).toLowerCase()
        )
      );
    })
  );

  public timezone$ = this.trudiDateFormat.timezone$;
  ...
  onFocus(event: FocusEvent, partType?: RangePartType): void {
    ...
    !this.nzIsSchedule && this.renderClass(true);
  }
  ...
  ngOnChanges(changes: SimpleChanges): void {
    const { nzStatus, nzPlacement, calendarEvent } = changes;
    ...
    if (calendarEvent?.currentValue) {
      const { eventName, date } = this.calendarEvent;
      this.datePickerService.calendarTypeToShow.next({
        eventName,
        date: this.datePickerService.makeValue(date as CompatibleDate)
      });
    }
  }
