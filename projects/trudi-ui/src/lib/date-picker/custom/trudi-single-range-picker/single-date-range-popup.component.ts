...
@Component({
  ...
  selector: 'single-date-range-popup',
  exportAs: 'singleDateRangePopup',
  template: `
    <ng-container *ngIf="isRange; else singlePanel">
      <div
        class="{{ prefixCls }}-range-wrapper {{
          prefixCls
        }}-date-range-wrapper">
        <div
          class="{{ prefixCls }}-range-arrow"
          [style.left.px]="datePickerService?.arrowLeft"></div>
        <div class="{{ prefixCls }}-panel-container">
          <div class="{{ prefixCls }}-{{ onlyLeft ? 'panel' : 'panels' }}">
            ...
            <ng-template #noTimePicker>
              <ng-container *ngIf="onlyLeft; else full">
                <div
                  *ngIf="inline"
                  class=" {{ prefixCls }}-panel d-flex flex-dir-column gap-12">
                  <div
                    class="d-flex flex-dir-column gap-12 justify-content-center {{
                      prefixCls
                    }}-header {{ prefixCls }}-custom-header align-items-center">
                    <div class="custom-header d-flex gap-12 align-items-center">
                      <ng-content></ng-content>
                    </div>
                    <div
                      class="d-flex justify-content-center"
                      [ngPlural]="getSelectedDays">
                      <ng-template ngPluralCase="=0"
                        >0 day is selected</ng-template
                      >
                      <ng-template ngPluralCase="=1"
                        >1 day selected</ng-template
                      >
                      <ng-template ngPluralCase="other"
                        >{{ getSelectedDays }} days selected</ng-template
                      >
                    </div>
                  </div>
                </div>
                <ng-container
                  *ngTemplateOutlet="
                    tplInnerPopup;
                    context: { partType: 'left', enableButton: ['next'] }
                  "></ng-container>
              </ng-container>
              <ng-template #full>
                <ng-container
    ...
    <ng-template
      ...
      <div
        ...
        <inner-popup
          ...
          (headerChange)="onActiveDateChange($event, partType)"></inner-popup>
      </div>
    </ng-template>
  ...
  host: {
    '[class.single-panel-range-picker]': `onlyLeft`
  }
})
export class SingleDateRangePopupComponent
  implements OnInit, OnChanges, OnDestroy, AfterViewInit
{
  ...
  //Customize input
  @Input() onlyLeft: boolean = false; //Only show left panel
  @Input() tplHeader: TemplateRef<HTMLElement>;
  @Input() inputValue!: string[];
  @Input() errorsInput!: boolean[];
  ...
  onActiveDateChange(value: CandyDate, partType: RangePartType): void {
    //Header emit active date change so we should handle base on current active input
    if (this.isRange && this.onlyLeft) {
      let activeDate: SingleValue[] = [];
      activeDate[this.datePickerService.getActiveIndex(this.activePart)] =
        value;
      this.datePickerService.setActiveDate(
        activeDate,
        this.hasTimePicker,
        this.getPanelMode(this.endPanelMode, this.activePart) as NormalizedMode
      );
      return;
    }
    if (this.isRange) {
  ...
  changeValueFromSelect(value: CandyDate, emitValue: boolean = true): void {
    if (this.isRange) {
      let selectedValue: SingleValue[] = cloneDate(
        this.datePickerService.value
      ) as CandyDate[];
      ...
      if (emitValue) {
        if (this.inline) {
          // For UE, Should always be reversed, and clear value when next part is right
          ...
          if (checkedPart === 'right' && !this.datePickerService.hasValue()) {
            selectedValue = cloneDate(
              this.datePickerService.value
            ) as CandyDate[];
            selectedValue[this.datePickerService.getActiveIndex('left')] =
              value;
            this.checkedPartArr = [true, false];
            nextPart = 'right';
          }

          if (checkedPart === 'right' && wrongSortOrder(selectedValue)) {
            selectedValue[this.datePickerService.getActiveIndex(nextPart)] =
              selectedValue[1];
            selectedValue[this.datePickerService.getActiveIndex(checkedPart)] =
              null;
            this.checkedPartArr[
              this.datePickerService.getActiveIndex(nextPart)
            ] = false;
  ...
  getActiveDate(partType?: RangePartType): CandyDate {
    if (this.isRange) {
      return this.onlyLeft
        ? (this.datePickerService.activeDate as CandyDate[])[
            this.datePickerService.getActiveIndex(this.activePart)
          ] ||
            (this.datePickerService.activeDate as CandyDate[])[
              this.datePickerService.getActiveIndex(
                this.reversedPart(this.activePart)
              )
            ]
        : (this.datePickerService.activeDate as CandyDate[])[
            this.datePickerService.getActiveIndex(partType)
  ...
  get getSelectedDays() {
    if (wrongSortOrder(this.datePickerService.value as CandyDate[])) return 0;
    return this.getValue('left') && this.getValue('right')
      ? Math.ceil(
          Math.abs(
            differenceInHours(
              startOfDay(this.getValue('left').nativeDate),
              endOfDay(this.getValue('right').nativeDate)
            ) / 24
          )
        )
      : 0;
